...
{
    struct nodo_s *next; // 1 => 0  asmdef_offset:NODO_OFFSET_NEXT
    uint8_t categoria;   // 1 => 1 asmdef_offset:NODO_OFFSET_CATEGORIA
    uint32_t *arreglo;   // 4 => 3 (padding 1)  asmdef_offset:NODO_OFFSET_ARREGLO
    uint32_t longitud;   // 4 => 8      asmdef_offset:NODO_OFFSET_LONGITUD
} nodo_t;                // => 16         asmdef_size:NODO_SIZE
